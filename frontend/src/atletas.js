    ...
    axios.get('http://localhost:8080/atletas')
        .then((response) => {
            ...
            atletasList.forEach(atleta => {
                grid.innerHTML +=
                    '<div class="flex flex-col bg-surface-container-low rounded-lg overflow-hidden group hover:translate-y-[-4px] transition-all">' +
                    ...
                    '<span class="bg-secondary-container/20 text-secondary px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider">' + (atleta.age || 'Desconocidos') + ' años</span>' +
                    '</div>' +
                    '<div class="flex flex-col gap-1">' +
                    '<div class="flex items-center gap-2 text-on-surface-variant text-sm">' +
                    '<span class="material-symbols-outlined text-xs">badge</span>' + (atleta.dni) +
                    '</div>' +
                    '<div class="flex items-center gap-2 text-on-surface-variant text-sm">' +
                    '<span class="material-symbols-outlined text-xs">call</span>' + (atleta.telephone || 'Sin teléfono') +
                    '</div>' +
                    '<div class="flex items-center gap-2 text-on-surface-variant text-sm">' +
                    '<span class="material-symbols-outlined text-xs">location_on</span>' + (atleta.city || 'Desconocida') +
                    '</div>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
