    ...
    axios.get('http://localhost:8080/carreras')
        .then((response) => {
            ...
            carrerasList.forEach(carrera => {
                ...
                grid.innerHTML += 
                    '<div id="carrera-' + carrera.id + '" class="flex flex-col bg-surface-container-low border border-outline-variant/30 rounded-lg p-8 group hover:border-secondary/50 transition-all min-h-[350px] justify-between">' +
                        ...
                        '<div class="flex flex-col gap-4">' +
                            '<div class="flex flex-col gap-4 border-t border-outline-variant/30 pt-6 mt-6">' +
                                ...
                                '</div>' +
                            '</div>' +
                            '<span class="text-white/5 font-headline font-black text-2xl self-end mt-2">' + carrera.id + '</span>' +
                        '</div>' +
                    '</div>';
            });
        })
...
window.removeCarrera = function(Id) {
    if(confirm('¿Estás seguro de que quieres eliminar esta carrera?')) {
        axios.delete('http://localhost:8080/carreras/' + Id)
            .then((response) => {
